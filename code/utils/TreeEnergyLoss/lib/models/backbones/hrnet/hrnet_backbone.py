...
from utils.TreeEnergyLoss.lib.models.tools.module_helper import ModuleHelper
from utils.TreeEnergyLoss.lib.utils.tools.logger import Logger as Log
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, bn_type, bn_momentum, in_chns=3, **kwargs):
        self.inplanes = 64
        ...
        if os.environ.get('full_res_stem'):
            ...
            stem_stride = 1
            self.conv1 = nn.Conv2d(in_chns, 64, kernel_size=3, stride=stem_stride, padding=1,
                                bias=False)
...
class HighResolutionNext(nn.Module):
    ...
    def __init__(self, cfg, bn_type, in_chns=3, **kwargs):
        super(HighResolutionNext, self).__init__()
        # stem net
        self.conv1 = nn.Conv2d(in_chns, 64, kernel_size=3, stride=2, padding=1,
                               bias=False)
...
class HRNetBackbone(object):
    ...
    def __call__(self):
        ...
        from utils.TreeEnergyLoss.lib.models.backbones.hrnet.hrnet_config import MODEL_CONFIGS

        if arch == 'hrnet18':
            arch_net = HighResolutionNet(MODEL_CONFIGS['hrnet18'], 
                                         bn_type='torchsyncbn',
                                         bn_momentum=0.1,
                                         in_chns=self.configer.get('network', 'in_chns'))
