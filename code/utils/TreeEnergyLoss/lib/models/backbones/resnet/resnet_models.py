...
from utils.TreeEnergyLoss.lib.models.backbones.resnet.wide_resnet_models import WiderResNetA2
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, deep_base=False, bn_type=None, in_chns=3):
        super(ResNet, self).__init__()
        self.inplanes = 128 if deep_base else 64
        if deep_base:
            self.resinit = nn.Sequential(OrderedDict([
                ('conv1', nn.Conv2d(in_chns, 64, kernel_size=3, stride=2, padding=1, bias=False)),
            ...
            self.resinit = nn.Sequential(OrderedDict([
                ('conv1', nn.Conv2d(in_chns, 64, kernel_size=7, stride=2, padding=3, bias=False)),
...
class ResNetModels(object):
    ...
    def resnet18(self, **kwargs):
        ...
        model = ResNet(BasicBlock, [2, 2, 2, 2], deep_base=False,
                       ...
                       in_chns=self.configer.get('network', 'in_chns'), **kwargs)
